...
- `601988` on Alpha Vantage (with a separate exchange parameter)
...
- **Market ID conflicts are common.** Multiple market types (spot, futures, perpetuals) can share the same exchange ID like `BTCUSDT`. CCXT resolves this with a global `defaultType` option, which means switching between spot and derivatives requires changing global state.
...
**OpenFIGI** is the closest to what we needed — a free, global identifier mapping service maintained by Bloomberg. But it's not open source, you can't self-host it, and its rate limits (currently 25 requests per minute for unauthenticated, 250 for authenticated) make it impractical for high-volume applications that need to resolve thousands of tickers during startup or bulk imports. You're also dependent on Bloomberg's infrastructure and data coverage decisions.

![OpenFIGI — free and flexible, but not open source and rate-limited for high-volume use](./OpenFigi.png)

**FinnWorlds** offers comprehensive market data APIs including exchange and listing metadata. But after a discounted first month ($9–$99), regular pricing kicks in at $99/month (Individual), $100/month (Starter), $400/month (Developers), and $1,100/month (Enterprise) — costs that add up quickly for personal projects, indie traders, or small teams who just need canonical ticker identity. It's also closed-source, so you can't customize the data model or add mapping rules for niche platforms.

![FinnWorlds pricing — four tiers from $99/month to $1,100/month after the first discounted month](./finnworldsPricing.png)
...
Our initial design centered on the **Market Identifier Code (MIC)** — the ISO 10383 standard maintained by SWIFT that assigns a unique code to every trading venue worldwide. Under this standard, every exchange has an **operating MIC** (the legal entity that operates the venue) and one or more **segment MICs** (specific trading segments or platforms within that venue). For example, `XNYS` is the operating MIC for New York Stock Exchange, Inc. Under it sit 11 segment MICs: `ARCX` (NYSE Arca), `XASE` (NYSE American), `XCHI` (NYSE Texas), `ARCO` (NYSE Arca Options), `AMXO` (NYSE American Options), `NYSD` (NYSE Dark), and more — each representing a distinct trading venue operated by the same entity.
...
But we quickly realized that many segment MICs under a single operating MIC share the same properties. NYSE's 11 segment MICs all share the same trading hours, the same geographic location (New York), the same timezone, and the same ticker suffixes across data platforms. Configuring trading hours, geo-location, and mapping rules for each segment MIC independently was redundant busywork.

We introduced a **Market** entity — a higher-level grouping defined by a short code (e.g., `NYSE`, `NASDAQ`, `LSE`, `SHG`) that maps to an operating MIC and absorbs all its segment MICs. The load process works by finding the operating MIC exchange record, walking up to the root of the parent hierarchy, collecting all descendants, and assigning every exchange in that tree to the same market. Instead of mapping `listing → MIC → data source`, we simplified to `listing → market → data source` and `data source → market → listing`. One fewer level of indirection.

The individual MICs are still stored and queryable — the `exchanges` table retains every MIC with its `isSegment` flag and `parentId` reference. But the mapping logic, trading hours, and geographic metadata all operate at the market level. Configure once per market, apply to all its MICs.

### Moving Mapping Rules to the Client

We originally planned to handle cross-platform symbol mapping inside TradingGoose Market itself — the server would know how to translate `AAPL` into `AAPL.US` for Finnhub or `0700.HK` for Yahoo Finance.

But we realized this was the wrong boundary. The canonical identity service should be concerned with **what exists** (which listings, on which markets, with which attributes), not with **how each data platform formats its symbols**. Platform-specific formatting is a concern of the client consuming the data — it changes when you add a new provider, and different clients might connect to different providers.
...
**Step 1 — Build context.** The listing's metadata is resolved from TradingGoose Market into a `ListingContext`: base ticker, quote currency, asset class, market code, country code, and city name. The provider's `marketToExchangeCode` map converts the market code into a platform-specific exchange code and suffix (e.g., market `NYSE` → exchange code `N` → suffix `.N`).

**Step 2 — Filter matching rules.** Every active rule is tested against the context. Each scope field in a rule acts as a filter — if a rule specifies `market: 'HKEX'`, it only matches when the listing's market is HKEX. If a rule specifies `city: 'SHANGHAI'` and `assetClass: 'stock'`, both must match. A rule with no scope fields matches everything (it's the catch-all fallback). If a `regex` field is present, it's tested against a source symbol string built from the context — for stocks this is just the base ticker, for crypto it's `{base}-{quote}`, for currency it's `{base}{quote}`.

**Step 3 — Score and rank.** Each matching rule is scored based on the precedence configuration for the current asset class. The score formula: for each scope field the rule specifies, add `precedence.length - index` where `index` is that field's position in the precedence array. Fields earlier in the precedence array carry more weight. Rules with a `regex` field get a +0.5 tiebreaker bonus.

For example, with stock precedence `['market', 'currency', 'country', 'city', 'listing']` (length 5):
...
The same listing — say, Bitcoin quoted in USD — resolves to `BTC-USD` for Yahoo, `BINANCE:BTCUSD` for Finnhub, and `BTC/USD` for Alpaca. The canonical identity (`TG_LSTG_XXXX`, base: `BTC`, quote: `USD`, assetClass: `crypto`) stays the same. Only the final rendered symbol changes per provider.

#### Why Inbound Data Doesn't Need Reverse Mapping

The key insight is that **the original canonical context is preserved throughout the request/response cycle**. When Studio sends a request for `601988.ss` to Yahoo Finance, it already knows this is listing `TG_LSTG_XXXX` with base `601988` on the Shanghai market. The response data — price bars, volume, timestamps — is associated back to the canonical listing directly. There's no need to parse `601988.ss` and reverse-engineer which listing it belongs to.
...
**TradingGoose Market is a self-hostable, open canonical market data platform that provides a single source of truth for listing identity, exchange metadata, and market reference data — designed so any trading application can resolve, search, and manage tickers consistently across data sources with configurable mapping rules.**

It manages eleven entity types — listings, exchanges, markets, cryptocurrencies, currencies, countries, cities, timezones, blockchain networks, and trading hours — through both an admin dashboard and a versioned public API.
...
The tech stack is deliberately simple: Next.js 16 on Bun, PostgreSQL with Drizzle ORM, Better Auth for sessions, and shadcn/ui for the admin interface. One deployable unit, one database, minimal operational overhead.

### Beyond Ticker Identity: Trading Hours, Holidays, and Early Closes

Once we had a relational system linking listings to markets, countries, cities, and timezones, we realized it could manage more than just ticker identity. Trading hours are a natural extension — they're tied to the same market entity, and every trading application eventually needs to know when a market is open.

TradingGoose Market stores trading hours as structured data: regular sessions by day of week, holiday closures, and early close dates with their shortened hours. The admin UI provides a visual weekly calendar where you can drag and configure sessions for each market, scoped by country, asset class, or even individual listing when needed (e.g., futures on CBOE have different hours than equities).
...
This is another area where the market-level grouping pays off. Instead of maintaining separate trading hour records for every segment MIC under NYSE, you configure it once for the NYSE market and it applies everywhere. If a specific asset class has different hours (like futures trading extended sessions), you add an override scoped to that asset class — the system resolves the most specific match.

For teams and individuals building trading applications that connect to multiple data sources, TradingGoose Market eliminates the fragmented ticker identity problem. You curate your listings once, define your mapping rules per platform, and every service in your stack speaks the same language. It's easy to scale, straightforward to manage, and affordable to run — because you host it yourself.
